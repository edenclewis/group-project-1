classdef updated_app_exported < matlab.ui.componentcontainer.ComponentContainer
...
WORDS_array = []; %% creates empty array to input the potential correct words into 
    ...
    for j = 1:5 %% goes through each letter of the word
        if upper(correct_word(j)) == upper(comp.edit_field.Value(j)) %% if the correct letter is equal to the guessed letter
    ...
    for j = 1:5 %% goes through each letter of the word
        ...
        if ismember(upper(comp.edit_field.Value(j)), upper(correct_word(green_letters == 0))) %% if the letter from the users guess is in the correct word but not in the correct place
